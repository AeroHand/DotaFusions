...
function DotaFusions:InitGameMode()
  ...
  DotaFusions.fusionList = fusionPlayers;
  ...
  Convars:RegisterCommand( "PlayerWantToKnowHeroSelection", function(...) return self:_PlayeSendHeroHero( ... ) end, "Player want to know hero selection", 0 )
  Convars:RegisterCommand( "EnableHiddenAbilitiesForSoloHeroes", function(...) return self:_EnableHiddenAbilitiesForSoloHeroes( ... ) end, "Player has opted to not use fusion", 0 )
  ...
  -- Load Hero Info KV file
  heroListKV = LoadKeyValues("scripts/npc/npc_heroes.txt")
  
  -- Load Melee hero ability swap list 
  meleeSwapList = LoadKeyValues("scripts/fusionKVs/meleeSwitchAblities.txt")
  
  -- Create a List for Melee heroes
...
function DotaFusions:SetupFusion(localPlayer, fusionHero)
    ...
    print("Setting up player " .. localPlayer:GetPlayerID().. " with " .. fusionHero)
    DotaFusions.fusionList[localPlayer:GetPlayerID()] = fusionHero;
    print(DotaFusions.fusionList[0])
    print(DotaFusions.fusionList[1])
    print(DotaFusions.fusionList[2])
    
                        PrecacheUnitByNameAsync(fusionHero, function()
                CreateUnitByName(fusionHero, Vector(-10000, -10000, 0), false, nil, nil, 0)
            end)
    ...
    if heroAbilities[fusionHero] then   
        ...
        DotaFusions.fusionList[localPlayer:GetPlayerID()] = nil
...
function DotaFusions:ProcessAbilityRequest(localPlayer, ability1, ability2, ability3, ability4, ability5, ability6)
    ...
    -- Get the fusion Hero
    local fusionHero = DotaFusions.fusionList[localPlayer:GetPlayerID()]
    
    if mainHero:GetUnitName() == fusionHero then
      print("Player has selected same hero, prevent fusion") return end
    ...
    print("Printing Modifiers Before")
    
    for i = 0, 16 do
        print(mainHero:GetModifierNameByIndex(i))
    end
    

    HideSpecialAbilities(mainHero)

    -- Cleares the current Hero abilties.
    ClearCurrentHeroAbilities(mainHero)
    -- Clears the current hero modifiers.  Three times to remove invoker mods
    ClearCurrentModifiers(mainHero)
    ClearCurrentModifiers(mainHero)
    ClearCurrentModifiers(mainHero)
    ...
    --HideSpecialAbilities(mainHero)
        ...
        print("Printing Modifiers After")
    
    for i = 0, 16 do
        print(mainHero:GetModifierNameByIndex(i))
    end
    
    --mainHero:FindAbilityByName("lina_fiery_soul"):OnOwnerSpawned()
...
function GetAbilityInfo(ability, kvTable, ...)
    ...
    for k,v in orderedPairs{...} do    
        ...
        if kvTable[string.lower(v)][ability] then       
            return ability, kvTable[string.lower(v)][ability]     
        end   
...
function ProcessAbilityIntoHero(abilityIndex, abilityInfo, hero)
   ...
   AddSingleAbility(abilityInfo.name, hero)
...
function AddAbilities(table, hero)
    ...
    for k,v in orderedPairs(table) do 
        ...
        AddSingleAbility(k, hero)

    end
end

function AddSingleAbility(name, hero)

    local abilityToAdd = name

    -- Is the unit melee, check for swap abilities
    if IsMeleeHero(hero:GetUnitName()) then
    
        if meleeSwapList[name] then abilityToAdd = meleeSwapList[name] end       
        
    end

    if hero:HasAbility(abilityToAdd) == false then
...
function ClearCurrentHeroAbilities(hero)
    ...
    for i = 0, 15 do   
        ...
        if currentAbi then
            ...
            elseif currentAbi:GetAbilityName() == "lina_fiery_soul" then         
...
function ClearCurrentModifiers(hero)

    hero:RemoveModifierByName("modifier_invoker_quas_instance")
    hero:RemoveModifierByName("modifier_invoker_exort_instance")
    hero:RemoveModifierByName("modifier_invoker_wex_instance")
    hero:RemoveModifierByName("modifier_invoker_quas_strength")
    hero:RemoveModifierByName("modifier_invoker_exort_intelligence")
    hero:RemoveModifierByName("modifier_invoker_wex_agility")
    hero:RemoveModifierByName("modifier_riki_permanent_invisibility")
    hero:RemoveModifierByName("modifier_death_prophet_exorcism")
    hero:RemoveModifierByName("modifier_death_prophet_exorcism_start_spirit_duration")
    hero:RemoveModifierByName("modifier_death_prophet_exorcism_start_time")
    hero:RemoveModifierByName("modifier_troll_warlord_berserkers_rage")
    hero:RemoveModifierByName("modifier_lina_fiery_soul")
      
end

function HideSpecialAbilities(hero)

    -- Visage
    if hero:FindAbilityByName("visage_soul_assumption"):IsHidden() == false then
        hero:FindAbilityByName("visage_soul_assumption"):SetLevel(1)  

        hero:SwapAbilities("fusion_visage_soul_assumption", "visage_soul_assumption", false, true)
        end

    hero:FindAbilityByName("visage_soul_assumption"):SetHidden(true)
    hero:FindAbilityByName("visage_soul_assumption"):SetAbilityIndex(13)
    
    --Ember Spirit
    if hero:FindAbilityByName("ember_spirit_fire_remnant"):IsHidden() == false then
        hero:FindAbilityByName("ember_spirit_fire_remnant"):SetLevel(1)  

        hero:SwapAbilities("fusion_ember_spirit_fire_remnant", "ember_spirit_fire_remnant", false, true)
        --hero:RemoveAbility("fusion_ember_spirit_fire_remnant")
    end
    
    hero:FindAbilityByName("ember_spirit_fire_remnant"):SetHidden(true)
    
    -- Lina
    if hero:FindAbilityByName("lina_fiery_soul"):IsHidden() == false then
    hero:FindAbilityByName("lina_fiery_soul"):SetLevel(1)  

    hero:SwapAbilities("fusion_lina_fiery_soul", "lina_fiery_soul", false, true)

    end
    
    hero:FindAbilityByName("lina_fiery_soul"):SetHidden(true)
    
    -- Meepo
    local meepoAbi = hero:FindAbilityByName("fusion_divided_we_stand_hidden")
    
    if meepoAbi ~= nil then
      meepoAbi:OnOwnerDied() end
      
    -- Death Prophet
    local deathProphetAbi = hero:FindAbilityByName("death_prophet_exorcism")
    
    if deathProphetAbi ~= nil then
      deathProphetAbi:OnOwnerDied() end
    ...
    for i = 0, 16 do
        ...
        if player:GetAbilityByIndex(i) ~= nil then
            ...
            if ability:GetName() == "ember_spirit_fire_remnant" then
                ...
                player:SwapAbilities(keys.ability:GetName(), "ember_spirit_fire_remnant", false, true)
                
                --player:RemoveAbility(keys.ability:GetName())
                
                if ability:GetLevel() < 1 then
                  ability:UpgradeAbility() end   
    ...
    for i = 0, 16 do
        ...
        if player:GetAbilityByIndex(i) ~= nil then
            ...
            if ability:GetName() == "visage_soul_assumption" then
                ...
                player:SwapAbilities(keys.ability:GetName(), "visage_soul_assumption", false, true)
...
-- Lina Fiery Soul Show hidden ability
function ShowFierySoulHidden(keys)
    ...
    for i = 0, 16 do
        ...
        if player:GetAbilityByIndex(i) ~= nil then
            ...
            if ability:GetName() == "lina_fiery_soul" then
                
                ability:SetHidden(false)
                
                player:SwapAbilities(keys.ability:GetName(), "lina_fiery_soul", false, true)
